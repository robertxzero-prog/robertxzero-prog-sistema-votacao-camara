...
export class AuthService {
  ...
  private base32Decode(input: string) {
    ...
    for (const char of sanitized) {
      ...
      if (index === -1) {
        throw new BadRequestException('Secret 2FA inválido.');
      }
  ...
  async login(email: string, senha: string, contexto?: LoginContexto & { twoFactorCode?: string }) {
    ...
    const acessoLicenca = await this.configuracaoService.validarAcessoPorLicenca();
    if (!acessoLicenca.ok) {
    ...
    if (
      ...
      throw new UnauthorizedException('Sistema temporariamente bloqueado por licença.');
    }
    ...
    if (precisa2fa) {
      ...
      if (!contexto?.twoFactorCode) {
        return {
          requires_2fa: true,
          message: 'Código 2FA obrigatório para este perfil.',
        };
      }
  ...
  async confirmarConfig2fa(authorization: string, code: string) {
    ...
    if (!usuario || !usuario.twofa_secret) {
      throw new BadRequestException('2FA não iniciado.');
    }
  ...
  validarToken(authorization: string) {
    if (!authorization) throw new UnauthorizedException('Token não informado');
